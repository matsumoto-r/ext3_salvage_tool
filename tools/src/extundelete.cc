...
bool commandline_duplicate = false;
...
//  os << "                         Show copies of inode 'ino' still in the journal.\n";
  ...
  os << " --debug                 output debug logs.\n";
  os << " --duplicate             If existing duplicate files with same inode, will restore\n";
  os << "                         duplicate files with same inode and write log for duplicate files.\n";
}
...
int decode_options(int& argc, char**& argv)
{
	...
	struct option longopts[] = {
		...
		{"duplicate", 0, &long_option, opt_duplicate},
		{NULL, 0, NULL, 0}
	};
		...
		int /*blocksize*/, char * /*buf*/, void *priv)
{
	...
	if(entry == DIRENT_DELETED_FILE || ps->del) {
        if (commandline_debug) {
            std::cout << "entry_iterate DIRENT_DELETED_FILE fname: " << dirent2->name << " inode: " << dirent2->inode << std::endl;
            std::cout << "entry_iterate inode check fname: " << dirent2->name << " inode: " << dirent2->inode;
            std::cout << " bigin: " << *((ps->inolist).begin()) << " end: " << *((ps->inolist).end()) << std::endl;
        }
		for( std::vector<ext2_ino_t>::iterator it=(ps->inolist).begin();
			...
			if(dirent2->inode == *it) {
                ...
                if (commandline_debug) {
                    std::cout << "entry_iterate list fname: " << dirent2->name << " inode: " << dirent2->inode << std::endl;
                    std::cout << "entry_iterate list fname: " << fname << " inode: " << dirent2->inode << std::endl;
                }
				restore_inode(ps->fs, ps->jfs, dirent2->inode, fname.c_str() );
                if (!commandline_duplicate)
				    (ps->inolist).erase(it);
				break;
			}
		}
		...
		std::vector<ext2_ino_t>& parent_inos )
{
	...
	if(retval == 0 && LINUX_S_ISDIR(inode->i_mode) ) {
		...
		for(unsigned long n = 0; n < numdatablocks(inode); n++) {
            ...
            if (commandline_debug)
                std::cout << "extundelete_process_dir_block " << " inode: " << ino << " dname: " << dirname << std::endl;
		}
		delete[] blocks;
	}
...
//*/
	...
	std::list<block_pair_t>::reverse_iterator rit;
	char *buf = new char[block_size_];
    int64_t deletefound_atime = -1;
    int64_t deletefound_dtime = -1;
	for ( rit=oldblks2.rbegin() ; rit != oldblks2.rend(); ++rit ) {
				...
				((int64_t) (inode->i_dtime)) <= commandline_before)
		{
			deletedfound = true;
            if (commandline_debug) {
                deletefound_atime = (int64_t) (inode->i_atime);
                deletefound_dtime = (int64_t) (inode->i_dtime);
			    std::cout << "deletefound atime is " << deletefound_atime << std::endl;
			    std::cout << "deletefound dtime is " << deletefound_dtime << std::endl;
            }
			continue;
		}
		if( deletedfound && inode_is_valid(inode)) {
		//if( deletedfound && deletefound_atime == (int64_t) (inode->i_atime) && inode_is_valid(inode)) {
			if(ver == 0) {
				found = true;
                if (commandline_debug) {
			        std::cout << "deletefound atime " << deletefound_atime << " match! next journal log inode atime is " << (int64_t) (inode->i_atime) << std::endl;
			        std::cout << "deletefound dtime " << deletefound_dtime << " match! next journal log inode dtime is " << (int64_t) (inode->i_dtime) << std::endl;
                }
				break;
			}
			ver--;
...
*/
			...
			if(!flag) {
				if (truncate( (outputdir + fname2).c_str(), EXT2_I_SIZE(inode)) == 0) {
                    ...
                    if (commandline_duplicate)
                        std::cout << ino << " " << inode->i_size << " Duplicated inode Check: " << (outputdir + fname2) << std::endl;
					retval = 0;
				} else {
